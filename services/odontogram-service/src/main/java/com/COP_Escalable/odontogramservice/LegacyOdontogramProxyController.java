package com.COP_Escalable.odontogramservice;
...
public class LegacyOdontogramProxyController {
	private final RestClient rest;
	private final String legacyBaseUrl;

	public LegacyOdontogramProxyController(RestClient.Builder rest, @Value("${app.legacy.base-url}") String legacyBaseUrl) {
		this.rest = rest.build();
		this.legacyBaseUrl = legacyBaseUrl;
	}

	@GetMapping(value = "/{patientId}", produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<String> get(@PathVariable String patientId, HttpServletRequest req) {
		return rest.get()
				.uri(legacyBaseUrl + "/api/odontogram/" + patientId)
	...
	@PatchMapping(value = "/{patientId}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<String> patch(@PathVariable String patientId, @RequestBody String body, HttpServletRequest req) {
		return rest.patch()
				.uri(legacyBaseUrl + "/api/odontogram/" + patientId)
				.headers(h -> h.addAll(forwardHeaders(req)))
